...
class r3_View extends View {
    ...
    public function get_xml_config($record_type_code, $config_type, $get_default_config=TRUE)
    {
        if (strtolower($config_type) == 'auto_lock_unlock')
            ...
            //Uu tien so 1st: File cau hinh chinh xac cua thu tuc (ex: TN01_config.xml)
            ...
            {
                //Uu tien so 2: Cau hinh cho linh vuc + cap (Ex: TNH00_config,xml..)
                ...
                {
                    //Uu tien so 3rd: Cau hinh cho linh vuc (Ex: TN00_config.xml)
    		        ...
    		        <td width="14%" style="text-align: left;">
                        ...
                        <td style="text-align: left;">
                            <input type="text" name="txt_record_type_code" id="txt_record_type_code"
                                            ...
                                            accesskey="1"
                            />
                            <select name="sel_record_type" id="sel_record_type" class="chosen" style="width:67.7%; color:#000000;"
            				onchange="sel_record_type_onchange(this)">
                                ...
                                <?php foreach ($arr_all_record_type as $code => $info):?>
                                    ...
                                    ?>
                                    <option value="<?php echo $code;?>"<?php echo $str_selected?> 
                                            data-mapping="<?php echo $v_mapping_code;?>" 
                                            data-scope="<?php echo $info['C_SCOPE'];?>"
                                            title="<?php echo $info['C_NAME'];?>"
                                            >
                                        <?php
	...
	public function render_form_display_all_record($data, $edit=TRUE) 
    {
        ...
        for ($i = 0; $i < sizeof($data); $i++)
        {
            ...
            //LienND update 2014-08-05: bat loi NULL database
            if (strlen($v_record_no) > 0)
            {
                ...
                $v_record_type_code = preg_replace('/^([A-Z0-9_]+)-([A-Z]+)-([A-Z0-9]+)$/', '$1', $v_record_no);
                ...
                if ($dom_workflow = simplexml_load_file($this->get_xml_config($v_record_type_code, 'workflow')))
                {
                ...
                else
                {
                    if (DEBUG_MODE > 0)
                    {
                        echo "$v_record_type_code: Sai XML workflow<br/>";
                                ...
                                elseif ($index == 'C_RETURN_DATE')
                                {
                                    ...
                                    if ($v_activity == 1)
                                    {
                                        ...
                                        elseif($data[$i]['C_PAUSE_DATE'] && !$data[$i]['C_UNPAUSE_DATE']){
                                            ...
                                            {
                                                //Da BIZ_DONE thi la "Da cho tra"
                                                if ($data[$i]['C_BIZ_DAYS_EXCEED'] != NULL)
                                                {
                                                    $append .= '<br><span class="days-remain">(Đã chờ trả ' . abs($v_return_days_remain) . ' ngày)</span></b>';
                                                }
                                                else
                                                {
                                                    //Chua BIZ_DONE thi la qua han
                                    ...
                                    else
                                    {
                                        if ($val == 1)
                                        {
                                                    ...
                                                    $v_next_role != _CONST_BO_SUNG_ROLE)
                                            {
                                                ...
                                                {
                                                        $val .= '<span class="days-remain flood">Đã chậm tiến độ ' . abs($v_step_days_remain) . ' ngày</span></b>';
                                                }
		...
		$html .= '<div class="page-notice">
                    <div id="notice">
                        <div class="widget-head blue">
                            <h3>
                                Thống kê hồ sơ
                                <span class="alert" style="padding: 8px !important; display: none">
                                    <i class="icon-exclamation-sign"></i>
                                    <span class="alert-content">Chậm tiến độ: 0 hồ sơ; Quá hạn: 0 hồ sơ;</span>
                                </span>
                            </h3>
                        </div>
    ...
    /**
     * lay ten file form_struct cua mot cong viec THU_LY bat buoc nhap ket qua thu ly (vd: TC06_result.xml)
     */
    public function get_require_form($record_type_code, $task_code = NULL)
    {
        if (empty($task_code))
        {
            $task_code = $record_type_code . _CONST_XML_RTT_DELIM . _CONST_THU_LY_ROLE;
        }
        $dom_flow = simplexml_load_file($this->get_xml_config($record_type_code,'workflow'));
        return trim(get_xml_value($dom_flow, "//task[@code='{$task_code}']/@require_form"));
    }

    /**
     * lay thoi gian toi thieu co the thuc hien hoàn thành thụ lý
     * @param type $record_type_code
     * @param string $task_code
     * @return type
     */
    public function get_minimum_time_to_exec($record_type_code, $task_code = NULL)
    {
        if (empty($task_code))
        {
            $task_code = $record_type_code . _CONST_XML_RTT_DELIM . _CONST_THU_LY_ROLE;
        ...
        $dom_flow = simplexml_load_file($this->get_xml_config($record_type_code,'workflow'));
        return trim(get_xml_value($dom_flow, "//task[@code='{$task_code}']/@minimum_time"));
    }

    
    
    //report
    function write_excel_report($objPHPExcel_Sheet) 
    {
        //$objPHPExcel_Sheet->
    }
