...
class record_Model extends Model {
            ...
            else
            {
                ...
                if ($this->db->getOne($stmt, $params) > 0)
                {
                    ...
                    if (get_role($v_next_task_code) == _CONST_KY_ROLE)
                    {
                        ...
                        {
                            $v_current_step_deadline_date = $this->db->getOne("Select C_DOING_STEP_DEADLINE_DATE From view_record Where PK_RECORD=$v_record_id");
                    
                            //So ngay thua ra:
                            $v_remain_days = $this->days_between_two_date($this->getDate(), $v_current_step_deadline_date);
                            if ($v_remain_days > 0)
                            {
                ...
                //Neu Rolde tiep theo la thu phi, tra ket qua ---> Danh dau hoan thanh nghiep vu
                ...
                if (strtolower($v_is_pause) == 'true' OR get_role($v_executing_task_code) == _CONST_THONG_BAO_BO_SUNG_ROLE)
                {
    ...
    private function _build_from_and_where_query_by_record_activity($p_activity_filter)
    {
        ...
        elseif ($p_activity_filter == 4)
        {
            ...
            $v_from_and_where = " view_processing_record Where
                                (   C_NEXT_TASK_CODE Like '%" . _CONST_XML_RTT_DELIM . _CONST_PHAN_CONG_ROLE . "'
                                    ...
                                    OR C_NEXT_TASK_CODE Like '%" . _CONST_XML_RTT_DELIM . _CONST_XAC_NHAN_HO_SO_LIEN_THONG_ROLE . "'
                                    OR C_NEXT_TASK_CODE Like '%" . _CONST_XML_RTT_DELIM . _CONST_CHUYEN_YEU_CAU_XAC_NHAN_XUONG_XA_ROLE . "'
                                ) And C_IS_PAUSING <> 1";
        }
        ...
        {
            //--9: HS qua han
            $v_from_and_where = ' view_processing_record Where C_BIZ_DAYS_EXCEED Is Null And (datediff(C_RETURN_DATE, Now()) < 0) And C_IS_PAUSING <> 1';
        }
        elseif ($p_activity_filter == 10)
        {
            //--10: Ho so dang tam dung: cho bo sung, nghia vu tai chinh
            $v_from_and_where = ' view_processing_record Where C_IS_PAUSING=1';
        }
        else
        {
            //Tat ca hs
    ...
    public function qry_all_record_by_role($role, $record_type_code='')
    {
        ...
        elseif (DATABASE_TYPE == 'MYSQL')
        {
            ...
            else
            {
                ...
                //Loc theo xã ?
                $v_xa_tiep_nhan = get_post_var('sel_can_bo_cap_xa','');
                if ($v_xa_tiep_nhan != '')
                {
                    $condition_query .= " And C_CREATE_BY='$v_xa_tiep_nhan'";
                }                
    ...
    */
    ...
    public function qry_all_notice($role)
    {
        ...
        elseif (strtoupper($role) == strtoupper(_CONST_TIEP_NHAN_ROLE))
        {
            ...
            $stmt  = "Select RT.C_CODE as record_type_code
                    ...
                    From t_r3_record_type RT Right Join (
                            ...
                            Where (C_LAST_TASK_CODE like '%$role' And C_CREATE_BY='$v_user_code')
                            Group By FK_RECORD_TYPE ) a
                        On RT.PK_RECORD_TYPE=a.FK_RECORD_TYPE
                    Order By RT.C_CODE";
    ...
    public function count_processing_record_by_role($role)
    {
        ...
        { 
            $task = '%' . _CONST_XML_RTT_DELIM . $v_real_role;
            $stmt = "Select ? role,  COUNT(*) count
                    From view_processing_record
                    Where C_LAST_TASK_CODE like '%$task' And C_CREATE_BY='$v_user_code'";
        }
        else
        {
            $stmt = "Select ? role,  COUNT(*) count
