...
for ($i=0; $i<$v_count; $i++)
{
    ...
    {
        $html .= '<tr>';
        $html .= '<td colspan="'.count($cols).'" class="group_name">' . $arr_all_group[$v_group_code]['C_NAME'] . '</td>';
        $html .= '</tr>';
        $j=0;
    }
    $j++;
...
switch ($v_format)
{
    ...
    case 'excel':
            ...
            $arr_unique_doing_group_code = array();
            foreach($arr_all_report_data as $arr_report_data)
            {
                if(!in_array($arr_report_data['C_DOING_GROUP_CODE'],$arr_unique_doing_group_code))
                {
                    $arr_unique_doing_group_code[] = $arr_report_data['C_DOING_GROUP_CODE'];
                }
            }
            $v_latest_row = 5 + count($arr_all_report_data) + count($arr_unique_doing_group_code);
            ...
            $objHTML->loadIntoExisting($v_html_file_name, $objPHPExcel,3);

            //Set border style

            $mysheet->getStyle("A4:{$v_latest_col}4")->applyFromArray($objHTML->styleArrayForHeader);
            $mysheet->getStyle("A5:{$v_latest_col}{$v_latest_row}")->applyFromArray($objHTML->styleArrayForData);
            //$cols
