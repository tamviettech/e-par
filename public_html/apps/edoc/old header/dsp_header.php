                    ...
                    <ul>
                        <?php if (isset($this->arr_roles)): ?>
                            ...
                            <script>
                                ...
                                {
                                    q = 'li[data-menu="1"]';
                                    var dont_count = ['<?php echo _CONST_TRA_CUU_ROLE ?>', '<?php echo _CONST_BAO_CAO_ROLE ?>', '<?php echo _CONST_Y_KIEN_LANH_DAO_ROLE ?>'];
                                    $(q).each(function(index) {
                                        var v_role = $(this).attr('data-role');
                                        
                                        if (dont_count.indexOf(v_role.toUpperCase()) == -1)
                                        {
                                            var url = '<?php echo $this->controller_url;?>'+ 'count_processing_record_by_role/' + v_role;
                                            if($(this).data('ajax_count')){
                                                $(this).data('ajax_count').abort(); //abort prev request
                                            }
                                            $(this).data('ajax_count') = $.ajax({
                                                url: url,
                                                cache: false,
                                                dataType: 'json',
