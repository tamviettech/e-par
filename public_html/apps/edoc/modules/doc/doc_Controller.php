...
class doc_Controller extends Controller
{
    ...
    {
        $file_id = replace_bad_char($file_id);
    }
    ...
    private function _dsp_all_doc($direction, $type)
    {
        ...
        $v_direct_ou_filter = isset($_POST['sel_direct_ou']) ? ($_POST['sel_direct_ou']) : -1;

        $v_type_filter = isset($_POST['type_filter']) ? ($_POST['type_filter']) : '';
        $v_begin_date  = isset($_POST['txt_begin_date']) ? ($_POST['txt_begin_date']) : '';
        $v_end_date    = isset($_POST['txt_end_date']) ? ($_POST['txt_end_date']) : '';
        ...
        $VIEW_DATA['direct_ou_filter']  = $v_direct_ou_filter;

        //Danh sach loại văn bản
        $VIEW_DATA['arr_all_list_type'] = $this->model->assoc_list_get_all_by_listtype_code('LOAI_VAN_BAN_TRONG_SO');
        $VIEW_DATA['v_type_filter']     = $v_type_filter;

        // tu ngay den ngay
        $VIEW_DATA['v_begin_date'] = $v_begin_date;
        $VIEW_DATA['v_end_date']   = $v_end_date;
    ...
    public function dsp_process($doc_id)
    {
        $doc_id = (int) $doc_id;
        $VIEW_DATA['arr_single_doc'] = $this->model->qry_single_doc($doc_id);
        $this->view->render('dsp_process', $VIEW_DATA);
    }
    ...
    public function dsp_print_document_book()
    {
        ...
        $this->view->render('dsp_print_document_book3', $VIEW_DATA);
    }
    ...
    /**
     * Quan tri danh sach NSD duoc chia se 1 van ban noi bo
     */
    public function dsp_doc_user($doc_id)
    {
        //$doc_id must be a integer
        if (!( preg_match('/^\d*$/', trim($doc_id)) == 1 ))
        {
        ...
        //Danh sach NSD da duoc chia se
        $arr_all_doc_user = $this->model->qry_all_doc_user($doc_id);

        $VIEW_DATA['arr_all_doc_user'] = $arr_all_doc_user;
        $VIEW_DATA['doc_id']           = $doc_id;
        $VIEW_DATA['xml_log']          = $this->model->db->getOne("Select C_XML_LOG From t_edoc_doc Where PK_DOC=$doc_id");

        $this->view->render('dsp_all_doc_user', $VIEW_DATA);
    }

    public function do_update_doc_user()
    {
        $this->model->do_update_doc_user();
    }

    public function dsp_print_tiep_dan_dinh_ky($doc_id)
    {
        //$doc_id must be a integer
        if (!( preg_match('/^\d*$/', trim($doc_id)) == 1 ))
        {
        ...
        $this->view->render('dsp_print_tiep_dan_dinh_ky', $VIEW_DATA);
    }

    public function dsp_print_tiep_dan_thuong_xuyen($doc_id)
    {
        //$doc_id must be a integer
        if (!( preg_match('/^\d*$/', trim($doc_id)) == 1 ))
        {
        ...
        $this->view->render('dsp_print_tiep_dan_thuong_xuyen', $VIEW_DATA);
    }

    /*
     * Hiển thị màn hình xử lý ban đầu văn bản đến
     */

    public function dsp_pre_processing()
    {
        //Kiem tra quyen
        if (!$this->check_permission('XU_LY_BAN_DAU_VAN_BAN_DEN'))
            die('Bạn không có quyền thực hiện chức năng này!');

        $doc_id = isset($_REQUEST['hdn_item_id']) ? Model::replace_bad_char($_REQUEST['hdn_item_id']) : 0;
        //$doc_id must be a integer
        if (!( preg_match('/^\d*$/', trim($doc_id)) == 1 ))
        {
            $doc_id = 0;
        ...
        $this->view->render('dsp_pre_processing_doc', $VIEW_DATA);
    }

    public function do_pre_processing_doc()
    {
        $this->model->do_pre_processing_doc();
    }

    public function view_attachment($file_id)
    {
        $this->model->view_attachment($file_id);
    }
