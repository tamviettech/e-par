...
<form name="frmMain" method="post" id="frmMain" action="<?php echo $this->get_controller_url();?>do_approve_doc/"><?php
    ...
    <div class="button-area">
        <input type="button" name="allot_onlick" class="button" value="Xác nhận" onclick="this.form.submit()"/>
        <?php $v_back_action = ($v_pop_win === '') ? 'btn_back_onclick();' : 'try{window.parent.hidePopWin();}catch(e){window.close();};';?>
        <input type="button" name="cancel" class="button" value="<?php echo _LANG_CLOSE_WINDOW_BUTTON; ?>" onclick="<?php echo $v_back_action;?>"/>
    </div>
</form>
