 ...
 *
 */
if (!defined('_PATH_TO_PEAR'))
	define('_PATH_TO_PEAR', SERVER_ROOT . 'libs/PEAR/');
require_once _PATH_TO_PEAR.'PEAR.php';
    ...
    function HTTP_Upload_Error($lang = null, $html = false)
    {
        ...
        $this->error_codes = array(
            'TOO_LARGE' => array(
                ...
                'vi'	=> "Kích thước file quá lớn. Độ lớn tối đa mà bạn có thể thực hiện upload là: $maxsize bytes."
                ),
            'MISSING_DIR' => array(
                ...
                'vi'	=> 'Không xác định được thư mục đích'
                ),
            'IS_NOT_DIR' => array(
                ...
                'vi'	=> 'Thư mục đích không tồn tại hoặc file có lỗi.'
                ),
            'NO_WRITE_PERMS' => array(
                ...
                'vi'	=> 'Bạn không có quyền ghi lên thư mục đích.'
            ...
            'NO_USER_FILE' => array(
                ...
                'vi'	=> 'Bạn chưa chọn file để upload.'
                ),
            'BAD_FORM' => array(
                ...
                'vi'	=> 'Form Upload không đúng qui định method="post" enctype="multipart/form-data".'
            ...
            'E_FAIL_COPY' => array(
                ...
                'vi'	=> 'Không thể copy được file tạm.'
                ),
            'E_FAIL_MOVE' => array(
            ...
            'CANNOT_OVERWRITE' => array(
                ...
                'vi'	=> 'File đã tồn tại và không cho phép ghi đè.'
                ),
            ...
            'DEV_NO_DEF_FILE' => array(
                ...
                'vi'	=> 'Tên file không xác định theo mẫu &lt;input type="file" name=?&gt;.'
                )
        );
    }
...
class HTTP_Upload_File extends HTTP_Upload_Error
{
    ...
    {
        $noalpha = '?É?ÓÚ?áéíóúýÂÊÎÔÛâêîôûÀÈÌÒÙàèìòùÄË?ÖÜäëïöüÿÃãÕõÅåÑñÇç@°ºª';
 ...
 *
 * @param string $file_obj Ten object upload file <input type="file" name="...">
 * @param string $upload_dir Ten thu muc tren Server chua file se upload
 * @param string $des_file_name Ten file sau khi Upload
 * @param string $lang Ngon ngu (Hien thi cac thong bao theo)
 * @return string Ten file sau khi upload
 *
 * @author Ngo Duc Lien <liennd@gmail.com>
 */
function upload_file($file_obj='userfile',$upload_dir='./',$des_file_name='real',$lang='en'){

	$upload = new http_upload($lang);
	$file = $upload->getFiles($file_obj);
	if (PEAR::isError($file)) {
		die ($file->getMessage());
	}
	if ($file->isValid()) {
		$file->setName($des_file_name);
		$dest_name = $file->moveTo($upload_dir);
		...
		return $dest_name;
			...
			<script language="javascript">
				alert('Khong tim thay file nguon hoac kich thuoc file lon qua!');
				window.goback();
			</script>
		<?php
		exit;
